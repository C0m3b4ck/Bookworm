...
class BookwormApp(tk.Tk):
    ...
    def add_new_book_form(self):
        ...
        form.grab_set()
        labels = {
            "EN": ["ID (0 may repeat)", "Title", "Author", "Year", "Genre", "Book Row"],
            "PL": ["ID (0 może się powtarzać)", "Tytuł", "Autor", "Rok", "Gatunek", "Regał książkowy"],
        }
        entries = {}
        ...
        def submit():
            ...
            book_row = entries[labels[self.lang][5]].get().strip()
            self.ws.append([id_val, title, author, year, genre, book_row])
            self.wb.save(self.file_name)
            messagebox.showinfo(
    ...
    def sort_by_column(self, tree, col):
        ...
        if filter_criteria:
            ...
            for row in rows:
                ...
                for key, val in filter_criteria.items():
                    if val:
                        col_i = {"ID": 0, "Title": 1, "Author": 2, "Year": 3, "Genre": 4}[key]
                        if key in ("ID", "Year"):
                            if str(val).lower() != str(row[col_i]).lower():
                                match = False
                                break
                        else:
                            if val.lower() not in str(row[col_i]).lower():
                                match = False
                                break
                if match:
                    filtered.append(row)
            rows = filtered
        col_index = {"ID": 0, "Title": 1, "Author": 2, "Year": 3, "Genre": 4}[col]
        rows.sort(key=lambda x: x[col_index] or "", reverse=not ascending)
        for row in rows:
            tree.insert("", "end", values=row)
