...
class BookwormApp(tk.Tk):
    ...
    def open_reader_panel(self):
        ...
        reader_combo = ttk.Combobox(
            assign_frame, textvariable=reader_var, state="readonly"
        )
        reader_combo.grid(row=0, column=1, padx=5, pady=5, sticky="w")
        ...
        book_combo = ttk.Combobox(assign_frame, textvariable=book_var, state="readonly")
        book_combo.grid(row=1, column=1, padx=5, pady=5, sticky="w")

        def load_readers_books():
            self.cursor.execute("SELECT id, name, surname, grade FROM readers")
            readers = self.cursor.fetchall()
            # Show full info in dropdown
            if self.lang == "EN":
                reader_combo["values"] = [
                    f"{r[0]}: {r[1]} {r[2]}, grade {r[3]}" for r in readers
                ]
            else:
                reader_combo["values"] = [
                    f"{r[0]}: {r[1]} {r[2]}, klasa {r[3]}" for r in readers
                ]
