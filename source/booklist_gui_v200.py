...
# Default themes (can add more in themes folder)
DEFAULT_THEMES = {
    "classic_blue": {
...
class BookwormApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Load themes (default + from folder)
        ...
        # If default language not set, ask now
    ...
    def set_language_and_start(self, lang):
        self.lang = lang
        # Save default language (ask user to save? We save it now)
    ...
    def create_main_menu(self):
        ...
        options = {
            "EN": ["Add New Book", "See/Modify Books", "Help", "Settings", "Exit"],
            "PL": ["Dodaj książkę", "Obejrzyj/modyfikuj książki", "Pomoc", "Ustawienia", "Zakończ"]
        }

        for i, option in enumerate(options[self.lang], start=2):
            btn = self.create_high_contrast_button(self, option, command=lambda choice=i - 2: self.handle_menu_choice(choice))
    ...
    def handle_menu_choice(self, choice):
        # choice mapping updated with 5 options
        if choice == 0:
    ...
    def add_new_book_form(self):
        ...
        for i, text in enumerate(labels[self.lang]):
            ...
            # Bind Enter to jump to next entry
            ent.bind("<Return>", lambda e, i=i: self.focus_next_entry(entries, labels[self.lang], i))
        ...
        # Bind Enter key on submit button to trigger submit
        submit_btn.bind("<Return>", lambda e: submit())

    def focus_next_entry(self, entries, label_list, current_index):
        # Move focus to next entry (below/right)
    ...
    def see_modify_books(self):
        ...
        search_labels_en = ["Search by ID", "Search by Title", "Search by Author", "Search by Year", "Search by Genre", "Search"]
        search_labels_pl = ["Szukaj po ID", "Szukaj po tytule", "Szukaj po autorze", "Szukaj po roku", "Szukaj po gatunku", "Szukaj"]
        ...
        self.sort_directions = {key: True for key in search_labels[:-1]}  # True = ascending, False = descending
        ...
        for idx, label in enumerate(search_labels[:-1]):
            ...
            ent.grid(row=0, column=idx*2+1, padx=(0, 10), sticky="w")
            entries_order.append(ent)
            # Bind Enter to move focus to next or search on last
            ent.bind("<Return>", lambda e, i=idx: self.focus_or_search(i, entries_order, filter_tree))
            # Bind Tab key default behavior (no override needed)

        btn_search = tk.Button(search_frame, text=search_labels[-1], bg=self.BTN_BG, fg=self.BTN_FG,
                               activebackground=self.BTN_HOVER_BG, activeforeground=self.BTN_HOVER_FG,
        ...
        def load_tree_data(filter_criteria=None, sort_by=None, ascending=True):
                ...
                for row in rows:
                    ...
                    for key, val in filter_criteria.items():
                        if val:
                            ...
                            # Special case for ID exact match and Year exact match for better UX:
                            if key in ("ID","Year"):
                                if str(val).lower() != str(row[idx]).lower():
                                    match = False
                                    break
                            else:
            ...
            # Sort
            if sort_by:
                col_idx = {"ID": 0, "Title": 1, "Author": 2, "Year": 3, "Genre": 4}[sort_by]
                # key function with type safe comparison for int/string
                def sort_key(r):
                    val = r[col_idx]
                    if val is None:
                        return ""
        ...
        def filter_tree():
            ...
            for key in self.search_vars:
                ...
                if val != "":
                    cleared_all = False
            # If all fields cleared, reset all
            if cleared_all:
                load_tree_data()
            else:
                # Translate keys to EN for filtering regardless of UI language
                keys_map_pl_en = {
                    "Szukaj po ID": "ID",
                ...
                for k, v in crit.items():
                    crit_en[keys_map_pl_en.get(k, k)] = v
        ...
        def on_remove():
            ...
            filter_tree()  # Refresh with current filters
    ...
    def sort_by_column(self, tree, col):
        ...
        # Translate current search criteria to English keys
        crit = {}
        cleared_all = True
        for key in self.search_vars:
            val = self.search_vars[key].get().strip()
        ...
        # Load data with current filter and sorting
        if cleared_all:
            filter_criteria = None
        else:
            filter_criteria = crit

        # Need access to tree and ws to load data, do a re-implementation of load_tree_data here
        tree.delete(*tree.get_children())
        rows = list(self.ws.iter_rows(min_row=2, values_only=True))
        # Filter
        if filter_criteria:
            ...
            for row in rows:
                ...
                for key, val in filter_criteria.items():
                    if val:
                        idx_ = {"ID": 0, "Title": 1, "Author": 2, "Year": 3, "Genre": 4}[key]
                        if key in ("ID","Year"):
                            if str(val).lower() != str(row[idx_]).lower():
                                match = False
                                break
                        else:
                            if val.lower() not in str(row[idx_]).lower():
                                match = False
                                break
                if match:
                    filtered.append(row)
        ...
        # Sort by column
        col_idx = {"ID": 0, "Title": 1, "Author": 2, "Year": 3, "Genre": 4}[col]
    ...
    def show_help(self):
        help_text_en = ("Bookworm Help:\n\n"
                        ...
                        "4. Settings: Change theme and default language.\n"
        ...
        help_text_pl = ("Pomoc Bookworm:\n\n"
                        ...
                        "4. Ustawienia: Zmień motyw i język domyślny.\n"
                        "5. Zakończ: Zamknij program.")
    ...
    def show_settings(self):
        ...
        def save_settings():
            ...
            messagebox.showinfo("Settings Saved" if self.lang == "EN" else "Zapisano ustawienia",
                                "Settings have been saved. Restart app to apply changes." if self.lang == "EN" else
                                "Ustawienia zostały zapisane. Uruchom ponownie aplikację, aby zastosować zmiany.")
            win.destroy()

        btn_save = tk.Button(win, text="Save" if self.lang == "EN" else "Zapisz",
                             bg=self.BTN_BG, fg=self.BTN_FG,
                             activebackground=self.BTN_HOVER_BG, activeforeground=self.BTN_HOVER_FG,
                             font=("Segoe UI", 12, "bold"),
